...
exports.getBillsByPo = exports.addSalesComment = exports.raisePoDispute = exports.raiseDispute = exports.createBill = exports.getBills = void 0;
...
const getBills = async (page = 1, limit = 20) => {
...
const createBill = async (payload) => {
    ...
    if (!purchaseOrderId)
        throw new Error("purchaseOrderId required in payload");
    const po = await postgres_1.default.purchaseOrder.findUnique({ where: { id: purchaseOrderId } });
    if (!po)
    ...
    const updatedPo = await postgres_1.default.purchaseOrder.update({ where: { id: purchaseOrderId }, data: { accountBills: [...existing, newBill], salesComments: salesComments ?? po.salesComments } });
    await Promise.all([
        redis_1.default.del(`purchase_orders:single:${purchaseOrderId}`),
        redis_1.default.del("purchase_orders:list:*"),
        redis_1.default.del("bills:list:*"),
        redis_1.default.del(`bills:by_po:${purchaseOrderId}`),
    ]);
    return newBill;
};
exports.createBill = createBill;
const raiseDispute = async (billId, employeeId, comments) => {
    ...
    for (const po of pos) {
        ...
        if (idx !== -1) {
            ...
            const newDispute = { id: (0, crypto_1.randomUUID)(), employeeId, comments, createdAt: new Date().toISOString() };
            bill.disputes = [...disputes, newDispute];
            bills[idx] = bill;
            await postgres_1.default.purchaseOrder.update({ where: { id: po.id }, data: { accountBills: bills } });
            await Promise.all([
                redis_1.default.del(`purchase_orders:single:${po.id}`),
                redis_1.default.del("purchase_orders:list:*"),
                redis_1.default.del("bills:list:*"),
                redis_1.default.del(`bills:by_po:${po.id}`),
            ]);
            return newDispute;
        }
    }
...
const raisePoDispute = async (poNo, employeeId, comments) => {
    ...
    await Promise.all([
        redis_1.default.del(`po:poNo:${poNo}`),
        redis_1.default.del(`purchase_orders:single:${po?.id}`),
        redis_1.default.del("purchase_orders:list:*"),
    ]);
    return newDispute;
...
const addSalesComment = async (poNo, comment) => {
    // Find the PO by poNo
    const po = await postgres_1.default.purchaseOrder.findUnique({ where: { poNo } });
    if (!po)
        throw new Error("PurchaseOrder not found");
...
const getBillsByPo = async (poId, page = 1, limit = 100) => {
    const cacheKey = getCacheKey("bills:by_po", { poId, page, limit });
    const cached = await redis_1.default.get(cacheKey);
    if (cached)
        return JSON.parse(cached);
    const po = await postgres_1.default.purchaseOrder.findUnique({ where: { id: poId }, include: { customer: true } });
    if (!po)
        throw new Error("PurchaseOrder not found");
    const bills = Array.isArray(po.accountBills) ? po.accountBills : [];
