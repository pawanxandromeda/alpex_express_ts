...
exports.MappingBuilder = exports.SchemaMapper = exports.DataNormalizer = exports.FuzzyMatcher = void 0;
/**
...
class FuzzyMatcher {
    static levenshteinDistance(str1, str2) {
        ...
        for (let i = 1; i <= b.length; i++) {
            for (let j = 1; j <= a.length; j++) {
                ...
                matrix[i][j] = Math.min(matrix[i][j - 1] + 1, // insertion
    ...
    static findBestMatch(searchTerm, candidates) {
        let bestMatch = null;
        ...
        for (const candidate of candidates) {
            ...
            if (normalizedCandidate.includes(normalizedSearch) ||
                ...
                const similarity = Math.max(normalizedSearch.length, normalizedCandidate.length) / Math.min(normalizedSearch.length, normalizedCandidate.length);
        ...
        for (const candidate of candidates) {
            const similarity = this.calculateSimilarity(normalizedSearch, candidate.toLowerCase());
            if (similarity > bestSimilarity) {
...
FuzzyMatcher.SIMILARITY_THRESHOLD = 0.7;
FuzzyMatcher.ABBREVIATIONS = {
...
class DataNormalizer {
     ...
     */
    static parseNumber(value, type = "float") {
    ...
    static parseBoolean(value) {
        ...
        return (["yes", "y", "true", "1", "active", "enabled", "on"].includes(str) ||
            null);
    }
    /**
...
class SchemaMapper {
    ...
    static mapAndValidate(rawData, fieldMappings) {
        const data = {};
        const errors = [];
        for (const [poField, sheetColumn] of Object.entries(fieldMappings)) {
            if (!sheetColumn || !(sheetColumn in rawData))
                continue;
...
class MappingBuilder {
     ...
     */
    static buildMapping(sheetHeaders) {
        const mapping = {};
        ...
        for (const poField of requiredFields) {
            ...
            for (const sheetHeader of sheetHeaders) {
                if (matchedHeaders.has(sheetHeader))
                    continue;
